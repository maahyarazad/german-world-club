...
/**
 * The ENGLISH public landing page.
 *
 * The same obligations as its German twin, asserted separately: a suite that
 * only checked `index.html` would pass against an English page that shipped a
 * bundle, lost its content or lied about its language.
 *
...
const HTML = readFileSync(join(process.cwd(), 'en.html'), 'utf8')
...
describe('the page renders without JavaScript', () => {
  ...
  it('carries its real content in the raw body', () => {
    expect(page.querySelector('h1')?.textContent).toMatch(/global network of trust/i)
    ...
    expect(text).toMatch(/pay-to-rank/i)
  })

  it('is German, and says so', () => {
    expect(page.documentElement.lang).toBe('en')
    expect(page.querySelector<HTMLMetaElement>('meta[property="og:locale"]')?.content).toBe('en_GB')
  })
})

describe('the English content mirrors the design document', () => {
  it('states the four value loops', () => {
    const text = page.body.textContent
    for (const loop of ['Ask GWC', 'Event', 'Benefit', 'Partner']) {
      expect(text).toContain(loop)
    }
    expect(text).toMatch(/Question → solution/)
    expect(text).toMatch(/Meeting → relationship/)
  })
  ...
  it('states the trust rules the document says are deliberately absent', () => {
    const text = page.body.textContent
    expect(text).toMatch(/No pay-to-rank/)
    expect(text).toMatch(/Member data is never sold/)
  })
})
...
describe('the login section reaches the console', () => {
  ...
  it('explains that one sign-in serves every role', () => {
    ...
    for (const role of ['Members', 'GWC team', 'Club Merchants', 'Corporate Club Partners']) {
...
describe('nothing of the previous brand survives', () => {
  ...
  it('names the German World Club instead', () => {
    ...
    expect(page.querySelector<HTMLLinkElement>('link[rel="canonical"]')?.href).toMatch(/german-world-club\.com\/en$/)
  })
})
