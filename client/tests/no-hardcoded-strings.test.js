 ...
 *
 * A component that imports `de.js` directly, or writes a German label inline,
...
describe('components read strings through the hook', () => {
  ...
  it('imports no catalogue directly', () => {
    // `import { t } from '../i18n/de.js'` hard-codes German into a component
    // and survives every switch. The hook is the only supported way in.
    const offenders = []
    for (const file of files()) {
      const source = readFileSync(file, 'utf8')
      if (/from ['"][^'"]*i18n\/(de|en)\.js['"]/.test(source)) {
  ...
  it('STILL CATCHES a planted direct import', () => {
    expect(/from ['"][^'"]*i18n\/(de|en)\.js['"]/.test("import { t } from '../i18n/de.js'")).toBe(true)
  })
})
