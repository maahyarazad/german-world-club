...
**Target Platform**: Gated member surface and staff console. No public surface.
...
| II. Declare Every Posture | **PASS** | Every route declares `config.auth`, enforced by the surviving boot gate (FR-019). `seo/surfaces.ts` already declares `/marketplace` gated and never-indexed, so the crawl posture is a *continuation*, not a new declaration (FR-020). Ownership is enforced against the loaded row inside the transaction (FR-003), which is exactly what the principle says route-level declarations cannot express. |
