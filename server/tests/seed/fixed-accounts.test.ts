...
describe.skipIf(!hasDatabase)('the demo seed leaves the fixed accounts exactly as they were', () => {
  ...
  beforeAll(async () => {
    ...
    await run('node', ['src/db/migrate.ts'], { cwd: SERVER, env: { ...process.env, ...env } })
    await run('node', ['src/scripts/seed-dev.ts'], { cwd: SERVER, env: { ...process.env, ...env } })
  ...
  it('still knows the password seed:dev actually sets', () => {
    const source = readFileSync(join(SERVER, 'src', 'scripts', 'seed-dev.ts'), 'utf8')
