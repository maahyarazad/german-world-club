...
export async function withSeededDatabase(name, { args = [] } = {}) {
  ...
  await run('node', ['src/db/migrate.js'], { cwd: SERVER, env })
  const { stdout } = await run('node', ['src/scripts/seed-demo.js', ...args], { cwd: SERVER, env })
...
export async function runSeed({ env = {}, args = [] } = {}) {
  try {
    const { stdout } = await run('node', ['src/scripts/seed-demo.js', ...args], {
      cwd: SERVER,
      env: { ...process.env, ...env },
    })
